	...
	if(a.type==1)
	{
		if(status==0)//там есть частицы склона
		{
			...
			for(int i=0;i<part_size;i++)
			{
				...
				{
					if((abs(a.cX-part[i].cX)>eps)&&(abs(a.cY-part[i].cY)>eps)&&(abs(a.cZ-part[i].cZ)>eps))
					{
...
void Bucket::bucForse(Particle& a,double re,double res[6])//производная частная пока
{
	...
	if((a.type==1)&&(status==0))//там есть частицы склона
	{
		for(int i=0;i<part_size;i++)
		{
			if((part[i].type==1)&&(abs(a.cX-part[i].cX)>eps)&&(abs(a.cY-part[i].cY)>eps)&&(abs(a.cZ-part[i].cZ)>eps))
			{
